...
from autocut.type import SPEECH_ARRAY_INDEX, LANG
...
class FasterWhisperModel(AbstractWhisperModel):
    ...
    def load(
        ...
        if not torch.cuda.is_available():
            self.device = "cpu"
        print("begin " + self.device)
        # self.whisper_model = WhisperModel(model_name, self.device)
        self.whisper_model = WhisperModel('./cykj/mode/large2/', self.device)
        print("over " + self.device)
